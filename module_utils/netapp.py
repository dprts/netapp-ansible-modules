...
def ontap_sf_host_argument_spec():
    ...
    return dict(
        ...
        style=dict(required=False, type='str', default='basic_auth'),
        cert_file=dict(required=False, type='str'),
        key_file=dict(required=False, type='str'),
    )


def ontap_sf_host_argument_required_if():

    return [
            ['style', 'basic_auth', ['username', 'password']],
            ['style', 'certificate_auth', ['cert_file', 'key_file']]
        ]
...
def setup_ontap_zapi(module, vserver=None):
    ...
    if HAS_NETAPP_LIB:
        ...
        if get_distribution('netapp_lib').version not in ['2015.9.25', '2016.10.14', '2017.10.30']:
